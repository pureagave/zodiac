...
- Street-aware routing: guidance follows how you actually drive BRC — free-drive across the open playa, but inside the city the route goes *playa → nearest entrance radial ∩ Esplanade → out to the ring → along the ring to the address*. The chevron + footer steer to the next corner (not straight-line), and the dashed route is drawn on both concepts (`core/navigation/PlayaRoute`).
