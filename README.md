...
- Drive-to navigation: pick a destination (HOME = camp at Heiau & 2:15, MAN = Golden Spike, TEMPLE, or BATH = nearest toilet bank, re-resolved live as you move) from a prominent full-width `DRIVE TO` button bar above the footer; the ops footer then shows bearing + distance + a heading-relative arrow to it.
- Heading-guidance chevron (MAP concept, second card): a big glance-and-steer indicator — a thick chevron rides a track, sliding to show how far off the active target is (`0.5 + Δ/360`: dead-ahead centred, 90° right at 75%, ±180° hard against an edge) and pointing the way to turn (► / ◄), recolouring to blue "ON COURSE" with an up chevron when you're lined up.
