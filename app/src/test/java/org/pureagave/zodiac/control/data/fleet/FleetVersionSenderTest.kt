...
/**
 * [FleetVersionSender] runs its loop on real `Dispatchers.IO`, so these tests
 * poll real wall-clock time (same style as `NavShareSenderTest`). The fixed
 * multicast group here is `239.7.7.40` (the version bus), not the nav bus.
 */
class FleetVersionSenderTest {
    @Test
    fun start_sends_immediately_then_at_the_rebroadcast_cadence_with_identical_bytes() =
        runBlocking {
            ...
            try {
                sender.start("\$ZVER,SENTENCE-A*00")
                ...
                assertTrue(
                    "every send must carry identical bytes",
                    socket.sent.all { it.second == "\$ZVER,SENTENCE-A*00" },
                )
            } finally {
                sender.stop()
                scope.cancel()
    ...
    fun each_tick_sends_to_both_targets_on_the_version_group() =
        runBlocking {
            val socket = FakeDatagramSocket()
            val scope = CoroutineScope(Dispatchers.IO + SupervisorJob())
            // No lease -> version group + limited-broadcast fallback.
            val sender =
                FleetVersionSender(
                    scope = scope,
                    rebroadcastMs = 5_000,
