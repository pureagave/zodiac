...
Jetson start emitting it — the ZTHREAT/ZCOVER discipline, applied from day one.

**`jetson/zvision/version_protocol.py`** — the Python mirror of the Kotlin
`FleetVersionProtocol`, written from the spec grammar (a genuine second
implementation, not a transliteration). Unlike ZTHREAT/ZCOVER (`;`-separated, no
checksum), `$ZVER` is an NMEA-style `$…*CC` sentence, so this carries its own XOR
checksum + the same six field regexes; `parse` never raises.

**`protocol/version-protocol-golden.json`** (6 format + 22 parse vectors) —
generated by `protocol/gen-version-golden.py`, a **third, independent** encoder
(inline XOR, explicit field join, imports no codec) so a vector only agrees with
an implementation if that implementation is correct. Negatives isolate one fault
each and — except `bad_checksum` — carry a *valid* checksum so the field grammar
is what's under test (pair every negative with a positive control). The
discriminating one: `signed_checksum_plus` = the correct checksum with a leading
`+`, which a naive `int(cc,16)`/`toInt(16)` accepts but the `[0-9A-Fa-f]` regex
guard rejects (the same trap the Kotlin `$ZVER` guard was written for).

**Both suites read the one file:** `FleetVersionProtocolGoldenTest.kt` and
`test_version_protocol_golden.py`, each failing loudly if the corpus is missing or
truncated (min-vector guards). Plus `test_version_protocol.py` for the Python
sanitiser/clamp branches the clean-input format vectors don't exercise. **Kotlin
and Python now validate byte-for-byte against the same 28 vectors.** No emitter is
wired yet (that's 6b); this is the contract. Gate green: `:app` **1067**,
`:beacon` 109, jetson **534**. ⚠️ Don't regenerate a red suite green — a mismatch
means an impl drifted; fix the impl. Beacon emit (fleet-fatal module) deliberately
**deferred for Rob's greenlight**.
...
failed camera's bearing arc identically to "that arc is clear" — for a safety
feature the dangerous ambiguity. Fixed by a dedicated, low-rate coverage channel
`ZCOVER`, per the spec's preferred design (`design/RES-P2-1-blind-arc-spec.md`).

**Wire design — separate channel, NOT a `ZTHREAT` extension.** `ZCOVER` rides the
*same* bus/socket as `ZTHREAT` (`239.7.7.20:10120`, through `ThreatBroadcaster`),
so no new socket/join/multicast-lock on the tablets. Payload is the union of arcs
covered by **currently-delivering** cameras: `ZCOVER;start:end;start:end` (degrees,
1 decimal; bare `ZCOVER` = whole ring blind). Covered arcs — not blind arcs — ride
the wire so the degenerate empty message fails BLIND, never clear. Chose this over
extending `ZTHREAT` because (1) coverage changes only when a camera dies/recovers,
so carrying it in every 10 Hz frame is waste, and (2) the `ZTHREAT` golden corpus
has no checked-in regenerator — touching it risked the exact drift it prevents.
**`threat_protocol.py` / `ThreatProtocol.kt` / `threat-protocol-golden.json` are
byte-untouched.** Backward compat is free both ways: an old tablet's
`ThreatProtocol.parse()` returns null for a `ZCOVER` datagram; an old Jetson never
sends one and the tablet's null-coverage path falls back to the static
`SurroundRing.COVERED_ARCS` (today's exact behaviour).

**Liveness, not config (R1/R2).** `CameraStallGuard` gained a `_last_frame`
timestamp advanced ONLY on a real frame — deliberately separate from `_last_ok`,
which the reopen backoff resets (reading that for liveness would make a dead
camera look alive for a stall interval after each reopen; pinned by a mutation
test). `delivering(window)` + `LIVE_WINDOW_SECS = 3.0 s` → a stalled camera drops
out of `rig.covered_arcs(detector.live_mounts())` within ~3 s + one loop tick.
`live_mounts()` excludes both non-delivering and this-frame-throwing cameras.

**New cross-language contract, differential discipline from day one.** `ZCOVER`
has its own golden corpus `protocol/coverage-protocol-golden.json`, GENERATED by a
checked-in tool `jetson/tools/gen_coverage_golden.py` (never hand-edited), read by
both suites. Numeric grammar / framing whitespace / f32 rounding shared verbatim
with `ZTHREAT` so the two channels can't drift on how they read a number.

**Tablet.** `NetworkThreatSource` parses `ZCOVER` into a `coverage` StateFlow with
its OWN 5 s staleness timer; it NEVER stamps `lastRxMs`/`feedAlive` — a
coverage-only stream can't resurrect a dead `ZTHREAT` feed, so `VisionFeed.ABSENT`
is preserved exactly (R4). `SurroundRingCoverage.rimSegments(feed, coverage)` (pure,
tested) classifies each rim arc COVERED (solid) / BLIND (dotted **red** — the
fault colour) / DEMO (dotted grey), with `normalizeCovered` folding the untrusted
wire list (overlap / containment / seam). `SurroundRingCanvas` maps style→paint
only. **Visible change:** today's single-thermal fleet now shows a large dotted-red
stern arc — intended (it *is* blind), and the `ZCOVER` value agrees with the static
fallback so old/new Jetsons don't flicker.

**Split, not bumped (post-implementation, human review).** The pipeline's first
cut raised the `SurroundRing` detekt `thresholdInObjects` 22→24 to fit the new
`rimSegments`/`normalizeCovered`/`uncoveredArcs`. Reverted per this repo's
"split the file, don't bump the number" rule (and the detekt comment's own "split
by concern (placement / alerting / coverage)"): the coverage classification moved
to a sibling `core/vision/SurroundRingCoverage` object — still pure, still tested,
`SurroundRing` back to 22. `COVERED_ARCS`/`isCovered` stay in `SurroundRing` as
...
RES-P2-1 R5 names `core/vision/SurroundRing` as the one home for the blind-arc
decision. Gates green, unscoped: jetson **523**, `:app` **1047**, `:beacon`
**109**. Left uncommitted for review.
