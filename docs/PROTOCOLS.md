...
### 3.7 `ZCOVER` — camera coverage / blind-arc signal (RES-P2-1)

A dead camera stops producing contacts for its bearing arc, and on the wire that
is **identical** to "that arc is clear." `ZCOVER` is the separate, low-rate
coverage channel that lets the surround ring tell *blind* from *clear*, so a
failed camera renders as a visible blind wedge, not a safe sector.

It rides the **same bus and socket as `ZTHREAT`** (`239.7.7.20:10120`, through
`ThreatBroadcaster`), so the tablets need no new socket, group join or multicast
lock — an old tablet's `ThreatProtocol.parse()` simply returns null for a
`ZCOVER` datagram and ignores it, and an old Jetson never sends one.

```
ZCOVER;<start>:<end>;<start>:<end>;...
```

One datagram is the set of bearing arcs a **currently delivering** camera can
see, swept clockwise, degrees to 1 decimal:
...
|---|---|
| Header | `ZCOVER` |
| Field / arc separators | `:` / `;` |
| `start` | bearing off the nose, `[-180, 180]` |
| `end` | `> start`, within one full turn (`end - start ≤ 360`); may exceed `+180` for a seam-straddling arc |
| Bare `ZCOVER` | **nothing covered — the whole ring is blind** |
| Max arcs | 16 (a malformed/over-count frame degrades toward *more* blind) |
| Grammar | same pinned `-?[0-9]{1,9}(\.[0-9]{1,6})?`, framing whitespace and 32-bit-float rounding as §3.3 |

**Covered arcs, not blind arcs, ride the wire, on purpose:** the degenerate empty
message then fails BLIND, never clear.

**Producer** (`zvision/coverage_protocol.py`): the covered set is
`rig.covered_arcs(detector.live_mounts())` — the union of arcs a camera that has
delivered a frame within `LIVE_WINDOW_SECS = 3.0 s` can see. A camera stalled,
unplugged or throwing drops out of `live_mounts()`, so its arc goes blind within
~3 s + one loop tick (R2). A `CoverageScheduler` emits at 1 Hz plus immediately
on any change; a bare `ZCOVER` (all blind) is sent on exit.

**Consumer** (`core/vision/CoverageProtocol.kt` → `NetworkThreatSource`): parses
into a `coverage` `StateFlow` with its **own** 5 s staleness timer. Coverage
**never** stamps threat liveness (`lastRxMs`/`feedAlive`) — a coverage-only
stream cannot resurrect a dead `ZTHREAT` feed, preserving §3.6's `ABSENT`
semantics exactly. `SurroundRing.rimSegments(feed, coverage)` classifies each rim
arc `COVERED` (solid) / `BLIND` (dotted red — the fault colour) / `DEMO` (dotted
grey). **Null coverage** (an old Jetson, or a stale signal) falls back to the
static `SurroundRing.COVERED_ARCS`, reproducing pre-RES-P2-1 behaviour.

`ZCOVER` is its own cross-language contract, so it has its own golden corpus,
`protocol/coverage-protocol-golden.json`, generated by
`jetson/tools/gen_coverage_golden.py` and read by both suites — the ZTHREAT
corpus is untouched.
